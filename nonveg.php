...
<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
        ...
        <input type="submit" name="submit" value="Add NonVeg"></div>
        </form>
<?php
$con= new mysqli("localhost","root","","restro");
if($con->connect_errno)
...
$dish=$_POST['dish'];
$price=$_POST['price'];
$column= "insert into nonveg values('$id','$dish','$price');";
...
<form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
        ...
        if(isset($_POST['search'])){
            $idd= $_POST['sid'];
        $search= "select dish,price from nonveg where id= $idd;";
        ...
        if(isset($_POST['delete'])){ 
            $idd= $_POST['sid'];
            $delete= "delete from nonveg where id= $idd;";
            $result = $con->query($delete);
        }
        ?>
        <form action="nonveg.php" method="post">
        <div class="refresh">
